...
public interface ICategoriaProductoRepository extends JpaRepository<CategoriaProducto, Long>, JpaSpecificationExecutor<CategoriaProducto> {
    @Query(value = "select cp from CategoriaProducto cp order by cp.codigo asc")
    List<CategoriaProducto> consultar();
    @Query(value = "select cp from CategoriaProducto cp where cp.estado=:estado order by cp.codigo asc")
    List<CategoriaProducto> consultarPorEstado(String estado);
    @Query(value = "select cp from CategoriaProducto cp where cp.abreviatura = :abreviatura and cp.estado=:estado order by cp.codigo asc")
