...
public interface IOperadorTarjetaRepository extends JpaRepository<OperadorTarjeta, Long>, JpaSpecificationExecutor<OperadorTarjeta> {
    @Query(value = "select ot from OperadorTarjeta ot order by ot.codigo asc")
    List<OperadorTarjeta> consultar();
    @Query(value = "select ot from OperadorTarjeta ot where ot.estado=:estado order by ot.codigo asc")
    List<OperadorTarjeta> consultarPorEstado(String estado);
	@Query(value = "select ot from OperadorTarjeta ot where ot.tipo=:tipo and ot.estado=:estado order by ot.codigo asc")
