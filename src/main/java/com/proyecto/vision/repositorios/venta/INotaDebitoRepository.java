...
public interface INotaDebitoRepository extends JpaRepository<NotaDebito, Long>, JpaSpecificationExecutor<NotaDebito> {
    @Query(value = "select nd from NotaDebito nd order by nd.id desc")
    List<NotaDebito> consultar();
    @Query(value = "select nd from NotaDebito nd where nd.estado = :estado order by nd.id desc")
    List<NotaDebito> consultarPorEstado(String estado);
    @Query(value = "select nd from NotaDebito nd where nd.empresa.id = :empresaId order by nd.id desc")
    List<NotaDebito> consultarPorEmpresa(long empresaId);
    @Query(value = "select nd from NotaDebito nd where nd.empresa.id = :empresaId and nd.estado = :estado order by nd.id desc")
    List<NotaDebito> consultarPorEmpresaYEstado(long empresaId, String estado);
    @Query(value = "select nd from NotaDebito nd where nd.factura.id = :facturaId and nd.empresa.id = :empresaId and nd.estado != :estado order by nd.id desc")
