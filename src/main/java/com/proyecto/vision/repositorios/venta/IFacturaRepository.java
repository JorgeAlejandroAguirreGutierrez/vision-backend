...
public interface IFacturaRepository extends JpaRepository<Factura, Long>, JpaSpecificationExecutor<Factura> {
    ...
    @Query(value = "select f from Factura f where (f.numeroComprobante like %:filtro% or f.cliente.razonSocial like %:filtro% or f.cliente.identificacion like %:filtro% or f.estado like %:filtro% or f.total like %:filtro%) and f.empresa.id = :empresaId order by f.id desc")
