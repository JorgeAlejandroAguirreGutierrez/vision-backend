...
public interface ICuentaContableRepository extends JpaRepository<CuentaContable, Long>, JpaSpecificationExecutor<CuentaContable> {
    @Query(value = "select cc from CuentaContable cc order by cc.id desc")
    List<CuentaContable> consultar();
    @Query(value = "select cc from CuentaContable cc where cc.empresa.id=:empresaId order by cc.id asc")
    List<CuentaContable> consultarPorEmpresa(long empresaId);
    @Query(value = "select cc from CuentaContable cc where cc.estado=:estado order by cc.id desc")
    List<CuentaContable> consultarPorEstado(String estado);
    @Query(value = "select cc from CuentaContable cc where cc.empresa.id=:empresaId and cc.estado = :estado order by cc.id asc")
