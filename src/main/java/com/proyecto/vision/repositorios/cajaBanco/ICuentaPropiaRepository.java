...
public interface ICuentaPropiaRepository extends JpaRepository<CuentaPropia, Long>, JpaSpecificationExecutor<CuentaPropia> {
    @Query(value = "select cp from CuentaPropia cp order by cp.id desc")
    List<CuentaPropia> consultar();
    @Query(value = "select cp from CuentaPropia cp where cp.estado=:estado order by cp.id desc")
    List<CuentaPropia> consultarPorEstado(String estado);
    @Query(value = "select cp from CuentaPropia cp where cp.empresa.id = :empresaId order by cp.id desc")
    List<CuentaPropia> consultarPorEmpresa(long empresaId);
    @Query(value = "select cp from CuentaPropia cp where cp.empresa.id = :empresaId and cp.estado=:estado order by cp.id asc")
    List<CuentaPropia> consultarPorEmpresaYEstado(long empresaId, String estado);
    @Query(value = "select cp from CuentaPropia cp where cp.banco.abreviatura = :banco and cp.empresa.id = :empresaId and cp.estado = :estado order by cp.id desc")
