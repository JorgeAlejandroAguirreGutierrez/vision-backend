...
public interface IKardexRepository extends JpaRepository<Kardex, Long>, JpaSpecificationExecutor<Kardex> {
    ...
    @Query(value = "select * from kardex k where k.producto_id = :productoId and k.bodega_id = :bodegaId and date(k.fecha) > :fecha order by date(k.fecha) asc, k.id", nativeQuery = true)
    ...
    @Query(value = "select * from kardex k where k.producto_id = :productoId and k.bodega_id = :bodegaId order by date(k.fecha) desc, k.id desc limit 1", nativeQuery = true)
