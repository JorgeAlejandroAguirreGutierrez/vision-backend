...
public interface IFormaPagoRepository extends JpaRepository<FormaPago, Long>, JpaSpecificationExecutor<FormaPago> {
    @Query(value = "select fp from FormaPago fp order by fp.codigo")
    List<FormaPago> consultar();
    @Query(value = "select fp from FormaPago fp where fp.estado=:estado order by fp.codigo")
    List<FormaPago> consultarPorEstado(String estado);
	@Query(value = "select fp from FormaPago fp where fp.codigo like '%'||:codigo||'%' and fp.descripcion like '%'||:descripcion||'%' and fp.abreviatura like '%'||:abreviatura||'%' order by fp.codigo")
