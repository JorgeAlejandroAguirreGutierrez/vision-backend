...
public interface ITransportistaRepository extends JpaRepository<Transportista, Long>, JpaSpecificationExecutor<Transportista> {
    @Query(value = "select t from Transportista t order by t.id desc")
    List<Transportista> consultar();
    @Query(value = "select t from Transportista t where t.estado= :estado order by t.id desc")
    List<Transportista> consultarPorEstado(String estado);
    @Query(value = "select t from Transportista t where t.empresa.id= :empresaId order by t.id desc")
    List<Transportista> consultarPorEmpresa(long empresaId);
    @Query(value = "select t from Transportista t where t.empresa.id = :empresaId and t.estado= :estado order by t.id asc")
